...
def extract_torso_and_gt(base_dir, ori_imgs_dir):
    ...
    for image_path in tqdm.tqdm(image_paths):
        ...
        # set mask
        mask = (head_part | neck_part | torso_part | inpaint_mask)
        if inpaint_torso_mask is not None:
            mask = mask | inpaint_torso_mask
        torso_image[~mask] = 0
        torso_alpha[~mask] = 0

        cv2.imwrite(image_path.replace('ori_imgs', 'face_imgs').replace('.jpg', '.png'), np.concatenate([torso_image, torso_alpha], axis=-1))
