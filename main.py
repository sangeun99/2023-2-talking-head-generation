...
if __name__ == '__main__':
    ...
    ## tori: for warping
    parser.add_argument('--use_warp', type=bool, default=True, help="whether to use the warp field")
    ## tori end
    ...
    parser.add_argument('--lip_shrink', type=float, default=0.8, help="shrink bg coords to allow more flexibility in deform")
    ...
    parser.add_argument('--density_thresh_lip', type=float, default=0.01, help="threshold for density grid to be occupied (alpha)")
    ...
    parser.add_argument('--lip', action='store_true', help="train lip")
    ...
    parser.add_argument('--ind_dim_lip', type=int, default=8, help="individual code dim, 0 to turn off")
    ...
    if opt.test:
        ...
        # ### tori : optimizer는 기존에 이용하던 것으로 이용
        # warp_scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, 
        #                                                       lambda step: 0.0 + (8.0 - 0.0) * (step / 80000))
        ...
        # trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, warp_scheduler=warp_scheduler, scheduler_update_every_step=True, metrics=metrics, use_checkpoint=opt.ckpt, eval_interval=eval_interval)
