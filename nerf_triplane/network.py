...
from nerfies import warping # tori: warping을 위한 모델 로딩
from nerfies import modules

from typing import Tuple
from nerfies import deformation
...
#     hidden_init = linen.initializers.xavier_uniform()
...
#     def __init__(self, dim_in, dim_out, dim_hidden, num_layers, skips=None, hidden_init=None, output_init=None):
#         super().__init__()
#         self.output_channels = dim_out
#         self.width = dim_hidden
...
#             # x = layer.init(random.PRNGKey(0), x)
#             variables = layer.init(jax.random.PRNGKey(0), x)
#             x = layer.apply(variables , x)
#             x = linen.relu(x)

#         if self.output_channels > 0:
#             logit_layer = linen.Dense(
#                 self.output_channels,
#                 use_bias=self.use_bias,
#                 kernel_init=self.output_init,
#                 name='logit')
#             variables = logit_layer.init(jax.random.PRNGKey(0), x)
#             x = logit_layer.apply(variables, x)
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        if self.lip:
            ...
            self.register_parameter('anchor_points', 
                                    nn.Parameter(torch.tensor([[0.01, 0.01, 0.1, 1], [-0.1, -0.1, 0.1, 1], [0.1, -0.1, 0.1, 1]])))
            self.lip_deform_encoder, self.lip_deform_in_dim = get_encoder('frequency', input_dim=2, multires=8)
            # self.lip_deform_encoder, self.lip_deform_in_dim = get_encoder('tiledgrid', input_dim=2, num_levels=16, level_dim=1, base_resolution=16, log2_hashmap_size=16, desired_resolution=512)
            self.anchor_encoder, self.anchor_in_dim = get_encoder('frequency', input_dim=6, multires=3)
            self.lip_deform_net = MLP(self.lip_deform_in_dim + self.anchor_in_dim + self.individual_dim_lip, 2, 32, 3)

            # lip color network
            self.lip_encoder, self.lip_in_dim = get_encoder('tiledgrid', input_dim=2, num_levels=16, level_dim=2, base_resolution=16, log2_hashmap_size=16, desired_resolution=2048)
            self.lip_net = MLP(self.lip_in_dim + self.lip_deform_in_dim + self.anchor_in_dim + self.individual_dim_lip, 4, 32, 3)
            # individual_dim_lip 는 renderer에 정의되어 있음
        ...
        if self.opt.use_warp:
            ...
            #                     max_freq_log2=None,
            #                     use_identity=True)
            # self.deform_net = MLP2(3, 3, 128, 6, (4, ), linen.initializers.xavier_uniform(), linen.initializers.uniform(scale=1e-4))
            self.deform_net = deformation.SE3DeformationField(deformation_field_config = SE3DeformationFieldConfig(),)
        ##### tori end #####
    ...
    def forward_lip(self, x, poses, c=None):
        # x: [N, 2] in [-1, 1]
        # head poses: [1, 4, 4]
        # c: [1, ind_dim], individual code

        # test: shrink x
        x = x * self.opt.lip_shrink
        ...
        # enc_pose = self.pose_encoder(poses)
        enc_anchor = self.anchor_encoder(wrapped_anchor)
        enc_x = self.lip_deform_encoder(x)
        ...
        dx = self.lip_deform_net(h)
        
        x = (x + dx).clamp(-1, 1)

        x = self.lip_encoder(x, bound=1)
    ...
    def forward(self, x, d, enc_a, c, e=None):
        ...
        # }
        # warp_metadata_encoder_type = 'time'
        # warp_metadata = (
        #     metadata['time']
        #     if warp_metadata_encoder_type == 'time' else metadata['warp'])
        # # print(warp_metadata.shape)
        # # print(*x.shape[:2])
        # warp_metadata = np.broadcast_to(
        #     warp_metadata[:, np.newaxis, :],
        #     shape=(*x.shape[:2], metadata_channels))
        # warp_extra = {
        #     'alpha': 0.0,
        #     'time_alpha': 0.0,
        # }
        ...
        #### tori ####
        # # x = self.points_encoder*/(x)
        # dx = self.deform_net(x)        
        # x = (x + dx).clamp(-1, 1)
    ...
    def get_params(self, lr, lr_net, wd=0):
        ...
        if self.lip:
            params = [
                {'params': self.lip_encoder.parameters(), 'lr': lr},
                {'params': self.lip_deform_encoder.parameters(), 'lr': lr, 'weight_decay': wd},
                {'params': self.lip_net.parameters(), 'lr': lr_net, 'weight_decay': wd},
                {'params': self.lip_deform_net.parameters(), 'lr': lr_net, 'weight_decay': wd},
                {'params': self.anchor_points, 'lr': lr_net, 'weight_decay': wd}
            ]

            if self.individual_dim_lip > 0:
                params.append({'params': self.individual_codes_lip, 'lr': lr_net, 'weight_decay': wd})
    ...
    # ## tori: nerfies > models.py > NerfModel > create_warp_field
    # # TODO: frame 수 찾아서 num_embeddings에 넣어주기
    # @staticmethod
    # def create_warp_field(model, num_batch_dims):
    #     return warping.create_warp_field(
    #         field_type='se3',
    #         num_freqs=8,
    #         num_embeddings=8000, # max(self.warp_ids) + 1 # frame 수 + 1, 현재 obama
    #         num_features=8,
