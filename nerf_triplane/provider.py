...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
            ...
            lip_img_path = os.path.join(self.root_path, 'lip_imgs', str(f['img_id']) + '.png')
            except_lip_img_path = os.path.join(self.root_path, 'except_lip_imgs', str(f['img_id']) + '.png')
            ...
            if self.preload > 0:
                lip_img = cv2.imread(lip_img_path, cv2.IMREAD_UNCHANGED) # [H, W, 4]
                lip_img = cv2.cvtColor(lip_img, cv2.COLOR_BGRA2RGBA)
                lip_img = lip_img.astype(np.float32) / 255 # [H, W, 3/4]

                self.lip_img.append(lip_img)
            else:
                self.lip_img.append(lip_img_path)

            if self.preload > 0:
                except_lip_img = cv2.imread(except_lip_img_path, cv2.IMREAD_UNCHANGED) # [H, W, 4]
                except_lip_img = cv2.cvtColor(except_lip_img, cv2.COLOR_BGRA2RGBA)
                except_lip_img = except_lip_img.astype(np.float32) / 255 # [H, W, 3/4]
                self.except_lip_img.append(lip_img)
            else:
                self.except_lip_img.append(except_lip_img_path)
        ...
        if self.preload > 0:
            ...
            self.lip_img = torch.from_numpy(np.stack(self.lip_img, axis=0)) # [N, H, W, C]
            self.except_lip_img = torch.from_numpy(np.stack(self.except_lip_img, axis=0)) # [N, H, W, C]
        else:
            self.images = np.array(self.images)
            self.torso_img = np.array(self.torso_img)
            self.lip_img = np.array(self.lip_img)
            self.except_lip_img = np.array(self.except_lip_img)
        ...
        if self.preload > 1:
            ...
            self.lip_img = self.lip_img.to(torch.half).to(self.device)
            self.except_lip_img = self.except_lip_img.to(torch.half).to(self.device)
    ...
    def collate(self, index):
        ...
        if self.training and self.opt.finetune_lips:
            ...
            rays = get_rays(poses, self.intrinsics, self.H, self.W, -1, self.opt.patch_size, rect=rect)
        else:
            rays = get_rays(poses, self.intrinsics, self.H, self.W, self.num_rays, self.opt.patch_size)
        ...
        # print(self.except_lip_img)
        bg_lip_img = self.except_lip_img[index]
        if self.preload == 0: # on the fly loading
            bg_lip_img = cv2.imread(bg_lip_img[0], cv2.IMREAD_UNCHANGED) # [H, W, 4]
            bg_lip_img = cv2.cvtColor(bg_lip_img, cv2.COLOR_BGRA2RGBA)
            bg_lip_img = bg_lip_img.astype(np.float32) / 255 # [H, W, 3/4]
            bg_lip_img = torch.from_numpy(bg_lip_img).unsqueeze(0)
        bg_lip_img = bg_lip_img[..., :3] * bg_lip_img[..., 3:] + self.bg_img * (1 - bg_lip_img[..., 3:])
        bg_lip_img = bg_lip_img.view(B, -1, 3).to(self.device)

        if self.opt.lip:      
            bg_img = bg_lip_img
        elif not self.opt.torso:
            bg_img = bg_torso_img
        else:
            bg_img = self.bg_img.view(1, -1, 3).repeat(B, 1, 1).to(self.device)
        ...
        if self.opt.lip and self.training:
            bg_lip_img = torch.gather(bg_lip_img, 1, torch.stack(3 * [rays['inds']], -1)) # [B, N, 3]
            results['bg_lip_color'] = bg_lip_img
