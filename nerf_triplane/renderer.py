...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        self.density_thresh_lip = opt.density_thresh_lip
        ...
        if self.lip:
            self.individual_dim_lip = opt.ind_dim_lip
            if self.individual_dim_lip > 0:
                self.individual_codes_lip = nn.Parameter(torch.randn(self.individual_num, self.individual_dim_lip) * 0.1) 
        ...
        # 2D lip density grid
        if self.lip:
            density_grid_lip = torch.zeros([self.grid_size ** 2]) # [H * H]
            self.register_buffer('density_grid_lip', density_grid_lip)
        self.mean_density_lip = 0
    ...
    def run_cuda(self, rays_o, rays_d, auds, bg_coords, poses, eye=None, index=0, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        lip_results = self.run_lip(rays_o, bg_coords, poses, index, bg_color)
    ...
    def run_lip(self, rays_o, bg_coords, poses, index=0, bg_color=None, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # auds: [B, 16]
        # index: [B]
        # return: image: [B, N, 3], depth: [B, N]
        ...
        # first mix lip with background
        if self.lip:
            # lip ind code
            if self.individual_dim_lip > 0:
                if self.training:
                    ind_code_lip = self.individual_codes_lip[index]
                # use a fixed ind code for the unknown test data.
                else:
                    ind_code_lip = self.individual_codes_lip[0]
            else:
                ind_code_lip = None
            
            # 2D density grid for acceleration...
            density_thresh_lip = min(self.density_thresh_lip, self.mean_density_lip)
            occupancy = F.grid_sample(self.density_grid_lip.view(1, 1, self.grid_size, self.grid_size), bg_coords.view(1, -1, 1, 2), align_corners=True).view(-1)
            mask = occupancy > density_thresh_lip

            # masked query of lip
            lip_alpha = torch.zeros([N, 1], device=device)
            lip_color = torch.zeros([N, 3], device=device)

            if mask.any():
                lip_alpha_mask, lip_color_mask, deform = self.forward_lip(bg_coords[mask], poses, ind_code_lip)

                lip_alpha[mask] = lip_alpha_mask.float()
                lip_color[mask] = lip_color_mask.float()

                results['deform'] = deform
            
            # first mix lip with background
            
            bg_color = lip_color * lip_alpha + bg_color * (1 - lip_alpha)

            results['lip_alpha'] = lip_alpha
            results['lip_color'] = bg_color

            # print(lip_alpha.shape, lip_alpha.max().item(), lip_alpha.min().item())
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        ### update density grid
        if not self.torso or not self.lip: # forbid updating head if is training torso...
        ...
        if self.torso:
         ...
         ### update lip density grid
        if self.lip:
            tmp_grid_lip = torch.zeros_like(self.density_grid_lip)
            ...
            if self.opt.ind_dim_lip > 0:
                ind_code = self.individual_codes_lip[[rand_idx]]
            else:
                ind_code = None
            ...
            for xs in X:
                for ys in Y:
                    ...
                    xys += (torch.rand_like(xys) * 2 - 1) * half_grid_size
                    # query density
                    alphas, _, _ = self.forward_lip(xys, pose, ind_code) # [N, 1]
                    
                    # assign 
                    tmp_grid_lip[indices] = alphas.squeeze(1).float()

            # dilate
            tmp_grid_lip = tmp_grid_lip.view(1, 1, self.grid_size, self.grid_size)
            # tmp_grid_lip = F.max_pool2d(tmp_grid_lip, kernel_size=3, stride=1, padding=1)
            tmp_grid_lip = F.max_pool2d(tmp_grid_lip, kernel_size=5, stride=1, padding=2)
            tmp_grid_lip = tmp_grid_lip.view(-1)
            
            self.density_grid_lip = torch.maximum(self.density_grid_lip * decay, tmp_grid_lip)
            self.mean_density_lip = torch.mean(self.density_grid_lip).item()

            # density_thresh_lip = min(self.density_thresh_lip, self.mean_density_lip)
            # print(f'[density grid lip] min={self.density_grid_lip.min().item():.4f}, max={self.density_grid_lip.max().item():.4f}, mean={self.mean_density_lip:.4f}, occ_rate={(self.density_grid_lip > density_thresh_lip).sum() / (128**2):.3f}')
    ...
    def render_lip(self, rays_o, rays_d, auds, bg_coords, poses, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # auds: [B, 29, 16]
        # eye: [B, 1]
        # bg_coords: [1, N, 2]
