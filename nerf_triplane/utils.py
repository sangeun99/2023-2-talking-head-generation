...
@torch.cuda.amp.autocast(enabled=False) # self.num_rays, self.opt.patch_size, rect=rect
...
class Trainer(object):
    def __init__(self, 
                 ...
                 warp_scheduler=None, # warp-scheduler
        ...
        if lr_scheduler is None:
            ...
            # self.warp_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda epoch: 1) # fake scheduler
        else:
            self.lr_scheduler = lr_scheduler(self.optimizer)
            # self.warp_scheduler = lr_scheduler(self.optimizer)
        ...
        self.log_ptr = None
        self.log("==========================")
        self.log(self.workspace)
        if self.workspace is not None:
            os.makedirs(self.workspace, exist_ok=True)        
            self.log_path = os.path.join(workspace, f"log_{self.name}.txt")
            self.log_ptr = open(self.log_path, "a+")
            self.log("==========================")
            self.log(self.log_ptr)
    ...
    def train_step(self, data):
        ...
        if not self.opt.torso and not self.opt.lip:
            rgb = data['images'] # [B, N, 3]
        elif self.opt.lip:
            rgb = data['bg_lip_color']
        else:
        ...
        if not self.opt.torso and not self.opt.lip:
            ...
            outputs = self.model.render_lip(rays_o, rays_d, auds, bg_coords, poses, eye=eye, index=index, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
        else:
        ...
        if self.opt.torso:
            ...
            alphas = outputs['lip_alpha'].clamp(1e-5, 1 - 1e-5)
            # alphas = alphas ** 2 # skewed entropy, favors 0 over 1
            loss_ws = - alphas * torch.log2(alphas) - (1 - alphas) * torch.log2(1 - alphas)
            loss = loss + 1e-4 * loss_ws.mean()
        else:
        ...
        if self.opt.amb_aud_loss and not self.opt.torso and not self.opt.lip:
        ...
        # eye att loss
        if self.opt.amb_eye_loss and not self.opt.torso and not self.opt.lip:
    ...
    def train_gui(self, train_loader, step=16):
        ...
        if not self.scheduler_update_every_step:
            if isinstance(self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                ...
                self.lr_scheduler.step()
            # if isinstance(self.warp_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
            #     self.warp_scheduler.step(average_loss)
            # else:
            #     self.warp_scheduler.step()
    ...
    def save_checkpoint(self, name=None, full=False, best=False, remove_old=True):
        ...
        state['mean_density_lip'] = self.model.mean_density_lip

        if full:
            state['optimizer'] = self.optimizer.state_dict()
            state['lr_scheduler'] = self.lr_scheduler.state_dict()
            # state['warp_scheduler'] = self.warp_scheduler.state_dict()
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        ...
        if 'mean_density_lip' in checkpoint_dict:
            self.model.mean_density_lip = checkpoint_dict['mean_density_lip']
        if model_only:
            return
        ...
        # if self.warp_scheduler and 'warp_scheduler' in checkpoint_dict:
        #     try:
        #         self.warp_scheduler.load_state_dict(checkpoint_dict['warp_scheduler'])
        #         self.log("[INFO] loaded warp scheduler.")
        #     except:
        #         self.log("[WARN] Failed to load scheduler.")
